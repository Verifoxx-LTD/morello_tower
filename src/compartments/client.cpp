...
COMPARTMENT_ENTRY_POINT(Password* __capability password) {
  ...
  // std::cout << "[Client]  Incoming Password: " << password->data << std::endl;
  KDF_Inputs input;
  memcpy_c(&input.passwd, password->data, sizeof(password->data));
  // strcpy(input.passwd, "dsbd_cheri");
  (void)getrandom(input.salt, sizeof(input.salt), 0);
  // strcpy(input.salt, "$123fvp_morello123$");
  std::cout << "[Client]" << std::endl;
  std::cout << "Incoming Password: " << input.passwd << std::endl;
  ...
  std::cout << "Generated (Random) Salt:" << std::endl;
  // PrintKdfInput(input);
  ...
  uintcap_t ret = CompartmentCall(kNodeOneCompartmentId, AsUintcap(input_cap), AsUintcap(client_derived_secret_cap));
  if (ret == 0) {
